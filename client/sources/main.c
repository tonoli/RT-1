...
int		main(int argc, char **argv)
{
	...
	color_array = (t_vector **)ft_memalloc(sizeof(t_vector *) * (F_HEIGHT + 1));
	frame_array = (int **)ft_memalloc(sizeof(t_vector *) * (F_HEIGHT + 1));
	int i = -1;
	while (++i < (int)F_HEIGHT)
	{
		color_array[i] = (t_vector *)ft_memalloc(sizeof(t_vector) * F_WIDTH);
		frame_array[i] = (int *)ft_memalloc(sizeof(int) * F_WIDTH);
	}
	...
	while (1)
	{
		...
		e->color_array = color_array;
		e->frame_array = frame_array;
		draw_screen(e);
