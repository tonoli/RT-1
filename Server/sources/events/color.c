...
/*   Updated: 2017/12/10 17:39:48 by itonoli-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	change_color_r(t_env *e, int mode)
{
	...
	if (!e->selected)
		return ;
	if ((mode == INCREASE || mode == 3) && e->selected->color.x < 0.99)
		e->selected->color.x += 0.005 * coef;
	else if ((mode == DECREASE || mode = 4) && e->selected->color.x > 0.01)
...
void	change_color_g(t_env *e, int mode)
{
	...
	if (!e->selected)
		return ;
	if ((mode == INCREASE || mode = 3) && e->selected->color.y < 0.99)
		e->selected->color.y += 0.005 * coef;
	else if ((mode == DECREASE || mode = 4) && e->selected->color.y > 0)
